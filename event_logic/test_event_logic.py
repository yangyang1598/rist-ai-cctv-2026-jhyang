...
class TestEventLogic:
    ...
    def test_universal_dynamic_event(self, cctv_name: str = "CCTV 1", detection_data: List[dict] = None):
        ...
        result = False
        if self.test_type == 61:
            # 안전대&안전고리 로직1 감지 테스트
            func_name = "safebelt1"
            result = self.call_event_function(func_name, detection_data, 0, 4, 5, 6)
        elif self.test_type == 62:
            # 안전대&안전고리 로직2 감지 테스트
            func_name = "safebelt2"
            result = self.call_event_function(func_name, detection_data, 0, 4, 5, 9)
        elif self.test_type == 7:
            # 보호구 감지 테스트
            func_name = "flash_suit"
            result = self.call_event_function(func_name, detection_data, 0, 2, 7)
        elif self.test_type == 8:
            # 월담 감지 테스트
            func_name = "waldam"
            result = self.call_event_function(func_name, detection_data, 0, 8)
        elif self.test_type == 9:
            # 쓰러짐 감지 테스트
            func_name = "falldown"
            result = self.call_event_function(func_name, detection_data, 0, 9)
    ...
    def run(self):
        ...
        try:
            ...
            if self.test_type == 61:
                # 안전대&안전고리 로직1
                class_filter = {0, 4, 5, 6} # 사람, 안전대, 안전고리, 안전고리 체결
                model = YOLO("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직1/best.pt", task="detect")
                cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직1/안전대 안전고리 미체결1.avi")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직1/안전대 안전고리 체결1.avi")
                # model = YOLO("grpc://localhost:8001/06_1_safebelt", task="detect")
                # cap = cv2.VideoCapture("rtsp://localhost:8554/061")

            elif self.test_type == 62:
                class_filter = {0, 4, 5, 9} # 사람, 안전대, 안전고리, 문열림
                # 안전대&안전고리 로직2
                model = YOLO("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직2/best.pt", task="detect")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직2/문이 열린상황, 안전대 안전고리 검지 필요1_gt.avi")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직2/문이 열린상황, 안전대 안전고리 검지 필요2_gt.avi")
                cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/6. 안전대&안전고리 로직2/문이 열린상황, 안전대 안전고리 검지 필요3_gt.avi")
                # model = YOLO("grpc://localhost:8001/06_2_open_door", task="detect")
                # cap = cv2.VideoCapture("rtsp://localhost:8554/062")

            elif self.test_type == 7:
                class_filter = {0, 2, 7} # 사람, 화재, 방염복
                # 보호구 감지 테스트
                # model = YOLO("/home/skysys/rist_model_test/스카이시스전달(250917)/7. 안전보호구/best.pt", task="detect")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/7. 안전보호구/안전보호구 검지O1.avi")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/7. 안전보호구/안전보호구 검지O2.avi")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/7. 안전보호구/안전보호구 검지X1.avi")
                model = YOLO("grpc://[::1]:8001/07_flash_check", task="detect")
                cap = cv2.VideoCapture("rtsp://localhosst:8554/07")

            elif self.test_type == 8:
                class_filter = {0, 8} # 사람, 월담
                # 월담 감지 테스트
                # model = YOLO("/home/skysys/rist_model_test/스카이시스전달(250917)/8. 월담/best.pt", task="detect")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/8. 월담/월담1.avi")
                # cap = cv2.VideoCapture("/home/skysys/rist_model_test/스카이시스전달(250917)/8. 월담/월담2.avi")
                model = YOLO("grpc://[::1]:8001/08_09_waldam_and_falldown", task="detect")
                cap = cv2.VideoCapture("rtsp://localhosst:8554/08")

            elif self.test_type == 9:
                class_filter = {0, 9} # 사람, 쓰러짐
                # 월담 감지 테스트
                model = YOLO("/home/skysys/rist_model_test/models/09_falldown.pt", task="detect")
                cap = cv2.VideoCapture("/home/skysys/rist_model_test/video/09_falldown/output.mp4")
                # model = YOLO("grpc://localhost:8001/08_09_waldam_and_falldown", task="detect")
                # cap = cv2.VideoCapture("rtsp://localhost:8554/08")
